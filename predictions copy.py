...
def lambda_handler(event, context):
    ...
    try:
        ...
        outputText = prediction(inputText)

        return {
            'statusCode': 200,
            'body': json.dumps({"bucket" : int(outputText)})
        }
...
def prediction(df):
    ...
    included_fields = ['setnumber', 'questionnumber', 'skill', 'level', 'verb_count_q', 'text_latex_stats', 'solution_latex_stats', 'text_len', 'latex_len', 
                       'text_len_question', 'latex_len_question', 'char_len_total_text', 'total_sol_len', 'steps','cluster']
    ...
    return rf_classifier.predict(X_input)[0]
